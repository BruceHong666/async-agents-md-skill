...
Every bug fix and every code-review thread hides a hard-won lesson. If those lessons live only in commit history and MR comments, the next AI agent walks the same minefield again. `async-agents-md` distills those lessons out of `fix` commits and review comments, dedups them against what is already in `agents.md`, and proposes concrete edits for a human to approve.
