...
每一次 bug 修复、每一轮 code review，背后都藏着一条来之不易的经验。如果这些经验只停留在提交历史和 MR 评论里，下一个 AI agent 还会踩同样的坑。`agents-md-keeper` 把这些经验从 `fix` 提交和 review 评论里提炼出来，与 `agents.md` 已有内容去重，再把具体的修改建议交给人工确认。
